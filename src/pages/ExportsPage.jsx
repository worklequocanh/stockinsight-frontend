...
export default function ExportsPage() {
  ...
  async function loadData(params = {}) {
    ...
    try {
      ...
      setError(parseApiError(err, 'Failed to load exports'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(parseApiError(err, 'Failed to create export receipt'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function approveExport(id) {
    ...
    try {
      ...
      setError(parseApiError(err, 'Failed to approve export'))
    }
  }
  ...
  async function rejectExport(id) {
    ...
    try {
      ...
      setError(parseApiError(err, 'Failed to reject export'))
    }
  }
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <p className="hero-copy">Manage warehouse outbound shipments (FEFO)</p>
      </div>
                  ...
                  {form.items.map((item, index) => (
                    <div key={index} style={{ border: '1px solid #e2e8f0', padding: '0.75rem', borderRadius: '4px' }}>
                      ...
                      <div className="two-col" style={{ marginBottom: '0.5rem' }}>
                        ...
                        <label>Unit Price <input type="number" min="0" step="0.01" className="field-input" value={item.unitPrice} onChange={(e) => updateItem(index, 'unitPrice', e.target.value)} required /></label>
                      </div>
                    </div>
                  ))}
                </div>
